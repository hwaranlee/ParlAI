...
from parlai.tasks.situational.agents import DefaultTeacher
...
def main():
    ...
    teacher = world.world.agents[0]
    domain_world_opt = copy.deepcopy(teacher.domain_teacher.opt)
    domain_world_opt['datatype'] = 'valid'
    domain_world = create_task(domain_world_opt, agent)
    '''
    domain_world = BatchWorld(opt,DialogPartnerWorld(
        opt, [DefaultTeacher(teacher.domain_teacher.opt), agent]))
        '''
    ...
    while True:
        ...
        #if True:
        if opt['log_every_n_parleys'] > 0 and \
                parleys % opt['log_every_n_parleys'] == 0:
        ...
        if opt['validation_every_n_parleys'] > 0 and \
            ...
            train_report, _ = run_eval(agent, opt, 'valid', opt['validation_max_exs'],
                    logger=logger, valid_world=domain_world)
            teacher.report_accuracy(train_report['accuracy'])
            if valid_report[opt['validation_metric']] - \
                    accuracy_weight * train_report['accuracy'] < best_loss:
                best_loss = valid_report[opt['validation_metric']] - \
                        accuracy_weight * train_report['accuracy']
                impatience = 0
                logger.info('[ new best loss: {}, accuracy: {} ]'.format(
                    best_loss, train_report['accuracy']))
                agent.save(model_save_path)
                saved = True
            else:
                impatience += 1
                logger.info(
                        '[ did not beat best loss: {} impatience: {} ]'.format(
            ...
            if opt['validation_patience'] > 0 and \
                    impatience >= opt['validation_patience']:
