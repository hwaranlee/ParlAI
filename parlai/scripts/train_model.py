...
def setup_args(parser=None) -> ParlaiParser:
  ...
  train.add_argument('--display-examples', type='bool',
...
class TrainLoop:
  ...
  def __init__(self, opt):
    ...
    self.log_every_n_secs = (
        opt['log_every_n_secs'] if opt['log_every_n_secs'] > 0 else float(
            'inf')
    )
    ...
    self.save_every_n_secs = (
        opt['save_every_n_secs'] if opt['save_every_n_secs'] > 0 else float(
            'inf')
    )
    ...
    if (
      ...
      print(
          '[ did not beat best {}: {} impatience: {} ]'.format(
              opt['validation_metric'], round(
                  self.best_valid, 4), self.impatience
          )
      )
