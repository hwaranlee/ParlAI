  ...
  def group_advance(self, word_lk, scores, score_mask, active_idx, active_idx_list):
    ...
    if len(active_idx_list) > 0:
      ...
      if len(self.prevKs) > 1:
        if word_lk.get_device() != score_mask.get_device():
          word_lk = word_lk.cuda(score_mask.get_device())
        beam_lk = score_mask.unsqueeze(1).expand_as(
            word_lk) * word_lk + scores.unsqueeze(1).expand_as(word_lk)
      ...
      # self.size ## active_idx_list ==> for each group!
      bestScores, bestScoresId = flat_beam_lk.topk(
          len(active_idx_list), 0, True, True)
      if scores.get_device() != bestScores.get_device():
        bestScores = bestScores.cuda(scores.get_device())
      ...
      if self.tt == torch.cuda:
        if prev_k.get_device() != torch.cuda.current_device():
          prev_k = prev_k.cuda()
        prev_k1 = torch.arange(0, self.size_g).long().cuda(
        ).scatter_(0, active_idx, active_idx[prev_k])
      else:
        prev_k1 = torch.arange(0, self.size_g).long().scatter_(
            0, active_idx, active_idx[prev_k])

      if next_ys.get_device() != torch.cuda.current_device():
        next_ys = next_ys.cuda()
      next_ys1 = self.tt.LongTensor(self.size_g).fill_(
          self.pad).scatter_(0, active_idx, next_ys)
    else:
      if self.tt == torch.cuda:
        prev_k1 = torch.arange(0, self.size_g).long().cuda()
      else:
    ...
    if len(self.prevKs) > 0:
      for g in range(self.group):
        scores, prev_k1, next_ys1 = self.group_advance(word_lk[g * self.size_g:(g + 1) * self.size_g],
                                                       self.scores[g * self.size_g:(
                                                           g + 1) * self.size_g],
                                                       self.score_mask[g * self.size_g:(
                                                           g + 1) * self.size_g],
                                                       # for each group
                                                       self.active_idx[g],
        ...
        prev_k[g * self.size_g:(g + 1) * self.size_g] = prev_k1 + \
            (g * self.size_g)  # adding the offset
        next_y[g * self.size_g:(g + 1) * self.size_g] = next_ys1
    else:
      scores, prev_k1, next_ys1 = self.advance(
          word_lk)  # select # group, copy-paste!!

      for g in range(self.group):
        self.scores[g * self.size_g:(g + 1) * self.size_g] = self.tt.FloatTensor(
            self.size_g).fill_(scores[g])
        prev_k[g * self.size_g:(g + 1) * self.size_g] = self.tt.LongTensor(
            self.size_g).fill_(prev_k1[g])  # consider the offset
        next_y[g * self.size_g:(g + 1) * self.size_g] = self.tt.LongTensor(
            self.size_g).fill_(next_ys1[g])
