...
class Beam(object):
  ...
  def advance_end(self, word_lk):
    ...
    # Sum the previous scores.
    if len(self.prevKs) > 0:
      beam_lk = self.score_mask.unsqueeze(1).expand_as(
          word_lk) * word_lk + self.scores.unsqueeze(1).expand_as(word_lk)
    ...
    bestScores, bestScoresId = flat_beam_lk.topk(
        len(self.active_idx_list), 0, True, True)  # self.size
    ...
    if self.tt == torch.cuda:
      prev_k1 = torch.arange(0, self.size).long().cuda().scatter_(
          0, self.active_idx, self.active_idx[prev_k])
    else:
      prev_k1 = torch.arange(0, self.size).long().scatter_(
          0, self.active_idx, self.active_idx[prev_k])

    next_ys1 = self.tt.LongTensor(self.size).fill_(
        self.pad).scatter_(0, self.active_idx, next_ys)
