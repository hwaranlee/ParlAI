...
class DictionaryAgent(Agent):
    ...
    def load(self, filename):
        ...
        """
        print('Dictionary: loading word dictionary from {}'.format(
              filename))
...
class CharDictionaryAgent(Agent):  # char2word
    """Builds and/or loads a dictionary.

    The char dictionary provides access to the frequency of each token, functions
    to translate sentences from tokens to their vectors (list of ints, each
    int is the index of a token in the dictionary) and back from vectors to
    tokenized text.
    """
    ...
    default_lang = 'english'
    default_maxngram = -1
    default_minfreq = 20 #0
    default_null = '__NULL__' # NULL_word = NULL_char
    default_start = '__START__' # START_word = START_char
    default_end = '__END__' # END_word = END_char
    default_unk = '__UNK__' # UNK_word = UNK_char
    ...
    def add_cmdline_args(argparser):
        ...
        dictionary.add_argument(
            ...
            help='path to a saved dictionary to load tokens / counts from to '
                 'seed the dictionary with initial tokens and/or frequencies')
        dictionary.add_argument(
            '--dict-language-char', default=CharDictionaryAgent.default_lang,
            help='sets language for the punkt sentence tokenizer')
        dictionary.add_argument(
            '--dict-max-ngram-size-char', type=int,
            default=CharDictionaryAgent.default_maxngram,
            help='looks for ngrams of up to this size. this is ignored when '
                 'building the dictionary. note: this takes approximate '
                 'runtime of len(sentence)^max_ngram_size')
        dictionary.add_argument(
            '--dict-minfreq-char', default=CharDictionaryAgent.default_minfreq,
            type=int,
            help='minimum frequency of words to include them in sorted dict')
        dictionary.add_argument(
           '--dict-nulltoken-char', default=CharDictionaryAgent.default_null,
           help='empty token, can be used for padding or just empty values')
        dictionary.add_argument(
          '--dict-starttoken-char', default=CharDictionaryAgent.default_start,
          help='token for starting sentence generation, if needed')
        dictionary.add_argument(
           '--dict-endtoken-char', default=CharDictionaryAgent.default_end,
           help='token for end of sentence markers, if needed')
        dictionary.add_argument(
            '--dict-unktoken-char', default=CharDictionaryAgent.default_unk,
            help='token to return for unavailable words')
        dictionary.add_argument(
            '--dict-maxexs-char', default=100000, type=int,
            help='max number of examples to build dict on')
        return dictionary
    ...
    def __init__(self, opt, shared=None):
        # initialize fields
        self.opt = copy.deepcopy(opt)
        self.null_token = opt['dict_nulltoken_char']
        self.end_token = opt['dict_endtoken_char']
        self.unk_token = opt['dict_unktoken_char']
        self.start_token = opt['dict_starttoken_char']
        self.max_ngram_size = opt['dict_max_ngram_size_char']
        ...
        if shared:
            ...
            if opt.get('dict_file_char') and os.path.isfile(opt['dict_file_char']):
                # load pre-existing dictionary
                self.load(opt['dict_file_char'])
            elif opt.get('dict_initpath_char'):
                # load seed dictionary
                self.load(opt['dict_initpath_char'])

        # initialize tokenizers
        st_path = 'tokenizers/punkt/{0}.pickle'.format(opt['dict_language_char'])
        try:
            self.sent_tok = nltk.data.load(st_path)
        except LookupError:
            nltk.download('punkt')
        ...
        if not shared:
            ...
            if opt.get('dict_file_char'):
                self.save_path = opt['dict_file_char']
    ...
    def tokenize(self, text, building=False):
        """Returns a sequence of tokens from the iterable."""
        return (list(token) for sent in self._sent_tokenize(text, building)
                for token in self._word_tokenize(sent, building))
        # just feed in list(token) to the tokenize function
    ...
    def add_to_dict(self, words):
        ...
        for word in words:
            for char in word:
                #pdb.set_trace()
                self.freq[char] += 1
                if char not in self.tok2ind:
                    index = len(self.tok2ind)
                    self.tok2ind[char] = index
                    self.ind2tok[index] = char
    ...
    def load(self, filename):
        ...
        Initialize counts from other dictionary, or 0 if they aren't included.
        """
        print('Dictionary: loading char dictionary from {}'.format(
              filename))
        with open(filename) as read:
            for line in read:
                split = line.strip().split('\t')
    ...
    def save(self, filename=None, append=False, sort=True):
        ...
        If ``sort`` (default ``True``), then first sort the dictionary before saving.
        """
        filename = self.opt['dict_file_char'] if filename is None else filename
        print('Dictionary: saving char dictionary to {}'.format(filename))
        if sort:
            self.sort()
