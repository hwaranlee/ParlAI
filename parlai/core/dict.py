...
class DictionaryCharAgent(Agent):
    """Builds and/or loads a dictionary.
    ...
    @staticmethod
    def add_cmdline_args(argparser):
        dictionary = argparser.add_argument_group('Dictionary Character Arguments')
        dictionary.add_argument(
            '--dict-char-file',
            help='if set, the dictionary will automatically save to this path during shutdown')
        dictionary.add_argument(
            '--dict-char-initpath',
            help='path to a saved dictionary to load tokens / counts from to ' +
                 'seed the dictionary with initial tokens and/or frequencies')
        '''
        dictionary.add_argument(
            '--dict-language', default=DictionaryCharAgent.default_lang,
            help='sets language for the punkt sentence tokenizer')
        dictionary.add_argument(
            '--dict-max-ngram-size', type=int,
            default=DictionaryCharAgent.default_maxngram,
            help='looks for ngrams of up to this size. this is ignored when ' +
                 'building the dictionary. note: this takes approximate ' +
                 'runtime of len(sentence)^max_ngram_size')
        dictionary.add_argument(
            '--dict-minfreq', default=DictionaryCharAgent.default_minfreq, type=int,
            help='minimum frequency of words to include them in the dictionary')
        dictionary.add_argument(
           '--dict-nulltoken', default=DictionaryCharAgent.default_null,
           help='empty token, can be used for padding or just empty values')
        dictionary.add_argument(
           '--dict-eostoken', default=DictionaryCharAgent.default_eos,
           help='token for end of sentence markers, if needed')
        dictionary.add_argument(
            '--dict-unktoken', default=DictionaryCharAgent.default_unk,
            help='token to return for unavailable words')
        '''
        dictionary.add_argument(
            '--dict-char-maxexs', default=100000, type=int,
            help='max number of examples to build dict on')
        return dictionary
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared:
            ...
            if opt.get('dict_char_file') and os.path.isfile(opt['dict_char_file']):
                # load pre-existing dictionary
                self.load(opt['dict_char_file'])
            elif opt.get('dict_char_initpath'):
                # load seed dictionary
                self.load(opt['dict_char_initpath'])
        ...
        if not shared:
            ...
            if opt.get('dict_char_file'):
                self.save_path = opt['dict_char_file']
    ...
    def tokenize(self, text, building=False):
        # For char dictionary, just return list of character sequence
        return list(text)
    ...
    def load(self, filename):
        ...
        Initialize counts from other dictionary, or 0 if they aren't included.
        """
        print('Dictionary: loading existing dictionary from {}'.format(filename))
        with open(filename) as read:
            for line in read:
                split = line.strip().split('\t')
                token = unescape(split[0])
                ...
                if token not in self.tok2ind:
                    ...
                    self.tok2ind[token] = index
                    self.ind2tok[index] = token
        print('[ num chars (before sortin & keeping) =  %d ]' % len(self))

    #def save(self, filename=None, append=False, sort=True):
    def save(self, filename=None, append=False, sort_and_keep100=True):
        """Save dictionary to file.
        Format is 'token<TAB>count' for every token in the dictionary, sorted
        by count with the most frequent words first.
    ...
    def sort_and_keep100(self):
        ...
        # sort first by count, then alphabetically
        sorted_pairs = sorted(self.freq.items(), key=lambda x: (-x[1], x[0]))
        sorted_pairs = sorted_pairs[0:100-1]
    ...
    def act(self):
        ...
        for source in ([self.observation.get('text')],
            ...
            if source:
                for text in source:
                    if text:
                        self.add_to_dict(self.tokenize(text)) # Since tokenize is just list of character seq, we can reuse this line
        return {'id': 'Dictionary'}
